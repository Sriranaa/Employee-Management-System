...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const resp = await axios.get('https://69f5fe38a72f01a951b915f3.mockapi.io/EmployeeData');
        this.list = resp.data;
      } catch (err) {
        console.error(err);
      }
    ...
    async updateData() {
      try {
        ...
        await axios.put(
          `https://69f5fe38a72f01a951b915f3.mockapi.io/EmployeeData/${this.editData.id}`,
          this.editData
        );
